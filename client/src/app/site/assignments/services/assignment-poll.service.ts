...
import { PercentBase, MajorityMethod } from 'app/shared/models/poll/base-poll';
...
export class AssignmentPollService extends PollService {
  ...
  public fillDefaultPollData(poll: Partial<ViewAssignmentPoll> & Collection): void {
      ...
      const length = this.pollRepo.getViewModelList().filter(item => item.assignment_id === poll.assignment_id).length;
